...
const REGISTRY_NAME = await fetchRegistryName();

async function fetchRegistryName() {
    const response = await axios.get(`${REGISTRY_URL}`, {
        headers: { Authorization: `Bearer ${TOKEN}` }
    });
    
    const REGISTRY_NAME = response.data.registry.name;
    console.log('Registry Names:', REGISTRY_NAME);
...
export async function main() {
    ...
    console.log("Repositories detected:");
